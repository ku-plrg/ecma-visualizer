...
function useTest262(db: IndexedDb) {
  const [selectedStep, setSelectedStep] = useState<SelectedStep>({
    ecId: null,
    step: null,
  });

  const [cToProgIds, setCToProgIds] = useState<Record<string, string>[] | null>(
    null,
  );
  ...
  const [selectedTest262, setSelectedTest262s] = useState<string[] | null>(
    null,
  );
  ...
  async function handleCallPathUpdate(): Promise<boolean> {
    ...
    const encodedString = cToProgIds[selectedFNCIdx][selectedCallPath];

    console.log("Encoded String", encodedString);
    const test262Ids = decode(encodedString);
    console.dir("Decoded Ids", test262Ids);
    ...
    setSelectedTest262s(test262Names);
    return true;
  }
